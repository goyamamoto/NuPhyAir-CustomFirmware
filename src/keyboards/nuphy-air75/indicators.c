...
// About a second: BAT_FLASH_SWEEPS lasts about a minute on the board.
#define USJIS_FLASH_SWEEPS 4
...
static uint8_t usjis_flash_sweeps;
static bool    usjis_flash_on;
...
#ifdef RF_ENABLED
    user_settings.rf_link = RF_MODE_BT1; // Bluetooth 1 after a flash or factory reset
#endif
#ifdef USJIS
...
// Shows a US-JIS mode change on the status half of the side lights.
void indicators_usjis_flash(bool enabled)
{
    usjis_flash_on     = enabled;
    usjis_flash_sweeps = USJIS_FLASH_SWEEPS;
}
...
static void led_regen_one()
{
    if (regen_row < LED_ROWS) {
        ...
        if (regen_col < UL_STATUS_COLS) {
            if (usjis_flash_sweeps) {
                // US-JIS turned on: magenta; turned off: dim white.
                r = usjis_flash_on ? 255 : 60;
                g = usjis_flash_on ? 0 : 60;
                b = usjis_flash_on ? 255 : 60;
            } else if (keyboard_state.led_state & (1 << 1)) {
                r = 50;
                g = 120;
                b = 50;
