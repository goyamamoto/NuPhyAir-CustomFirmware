...
#endif
#ifdef USJIS
    usjis_set_win_mode(user_keyboard_state.os_mode == KEYBOARD_OS_MODE_WIN);
#endif
}
...
void kb_update_switches()
{
    ...
    // The stock firmware keeps P7.6 at the inverse of the USB-power pin.
...
extern void indicators_usjis_flash(bool enabled);
...
#ifdef RF_ENABLED
// Timed in scans rather than main-loop passes (60000 passes took about a
// minute on the board in wireless mode). 4400 scans measured about 6 s on an
// Air75, which is the spec; the stock firmware pairs after 3-4 s.
#    define LINK_PAIRING_HOLD_SCANS 4400
...
{
    indicators_usjis_flash(enabled);
}
#endif
